...
public class CryptoController {
    ...
    /**
     * Deletes a cryptocurrency from the portfolio by its ID.
     * 
     * @param id the ID of the cryptocurrency to delete
     * @return ResponseEntity with appropriate status code or NOT_FOUND if the crypto does not exist
     */
    @DeleteMapping("/cryptos/{id}")
    public ResponseEntity<Crypto> deleteCrypto(@PathVariable int id) {
        Crypto existingCrypto = cryptoService.getCryptoById(id);
        if (existingCrypto == null) {
            return ResponseEntity.status(HttpStatus.NOT_FOUND).build();
        }
        cryptoService.deleteCrypto(id);
        return ResponseEntity.ok(existingCrypto);
    }
}
